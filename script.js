...
function elemenateBlackPeice(){
  if(chessboard[tlocA][tlocB].innerHTML!=""){
    document.getElementById('elemenatedBlackPeiceSet').innerHTML+=chessboard[tlocA][tlocB].innerHTML;
  }
  }
function elemenateWhitePeice(){
  if(chessboard[tlocA][tlocB].innerHTML!=""){
  document.getElementById('elemenatedWhitePeiceSet').innerHTML+=chessboard[tlocA][tlocB].innerHTML;}
}

function updateMove(peice){
  if(peice.includes('Wpeice')){
    Movedata.innerHTML=(`<li style="background-color:#65656d; color: #f1f1f2;"> ${alphabetPart[pi] || `castling`}${pj+1}${peice}${alphabetPart[tlocB]}${tlocA+1}</li>`+Movedata.innerHTML);
  }
  else{
    Movedata.innerHTML=(`<li style="background-color:#65656d; color: #f1f1f2;"> ${alphabetPart[pi]}${pj+1}${peice}${alphabetPart[tlocB]}${tlocA+1}</li>`+Movedata.innerHTML);
  }
  turn = !turn;
}
function checkking(){
